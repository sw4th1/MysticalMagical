...
(async () => {
    if (!code) {
        ...
        try {
            ...
            const btn = document.getElementById("showTopTracks")!;
...
async function fetchTopTracks() {
    ...
    const res = await fetch("https://api.spotify.com/v1/me/top/tracks?limit=50&time_range=short_term", {
        headers: { Authorization: `Bearer ${token}` }
    });
...
function renderTopTracks(tracks: any[]) {
    ...
    tracks.forEach(track => {
        ...
        li.textContent = `${track.name} by ${track.artists.map((a: any) => a.name).join(", ")}`;
        list.appendChild(li);
    });
}
